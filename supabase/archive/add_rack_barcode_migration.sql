...
-- Use ROW_NUMBER to ensure unique barcodes even if rack_numbers are duplicated across sessions
UPDATE racks 
SET barcode = 'RACK' || LPAD(
  (ROW_NUMBER() OVER (ORDER BY audit_session_id, rack_number))::text, 
  3, 
  '0'
)
