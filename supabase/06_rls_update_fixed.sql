-- Row Level Security Update for Username/Password Authentication (FIXED)
...
CREATE POLICY "users_view_location_racks" ON racks
  FOR SELECT USING (
    ...
    EXISTS (
      ...
      WHERE audit_sessions.id = racks.audit_session_id 
      AND audit_sessions.location_id = ANY((SELECT location_ids FROM users WHERE auth.uid() = id))
    )
  );
...
CREATE POLICY "users_view_location_scans" ON scans
  FOR SELECT USING (
    ...
    (
      ...
      EXISTS (
        SELECT 1 FROM audit_sessions 
        WHERE audit_sessions.id = scans.audit_session_id 
        AND audit_sessions.location_id = ANY((SELECT location_ids FROM users WHERE auth.uid() = id))
      )
    )
  );
