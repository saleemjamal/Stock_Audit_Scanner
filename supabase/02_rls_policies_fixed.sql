...
CREATE POLICY "Users can view locations they have access to" ON locations
    FOR SELECT USING (
        auth.uid() IN (
            ...
            WHERE u.id = auth.uid() 
            AND (u.location_ids @> ARRAY[locations.id] OR u.role = 'admin')
        )
    );
...
CREATE POLICY "Supervisors can view users in their locations" ON users
    FOR SELECT USING (
        auth.uid() IN (
            ...
            AND (u.location_ids && users.location_ids OR u.role = 'admin')
        )
    );
...
CREATE POLICY "Users can view audit sessions for their locations" ON audit_sessions
    FOR SELECT USING (
        auth.uid() IN (
            SELECT u.id FROM users u
            WHERE u.id = auth.uid() 
            AND (u.location_ids @> ARRAY[audit_sessions.location_id] OR u.role = 'admin')
        )
    );
...
CREATE POLICY "Users can view racks for their locations" ON racks
    FOR SELECT USING (
        auth.uid() IN (
            SELECT u.id FROM users u
            WHERE u.id = auth.uid() 
            AND (u.location_ids @> ARRAY[racks.location_id] OR u.role = 'admin')
        )
    );
...
CREATE POLICY "Users can view scans for their locations" ON scans
    FOR SELECT USING (
        auth.uid() IN (
            ...
            WHERE u.id = auth.uid() AND r.id = scans.rack_id
        )
    );
